...
**Domain:** Windows 11 Shell Extension / Context Menu Integration
**Researched:** 2026-04-08
**Confidence:** HIGH (verified against official Microsoft docs, real-world implementations)

---

## The Core Constraint

The Windows 11 modern (top-level) context menu **requires a COM in-process DLL** implementing `IExplorerCommand` + package identity established via a sparse MSIX package. There is no shortcut: no DLL means no top-level placement — the entry falls under "Show more options" (the classic/legacy menu).

This is confirmed by official Microsoft docs, the VS Code reference implementation (`vscode-explorer-command`), and community implementations (Notepad++, NanaZip, xplorer²).

The `uap2:SupportedVerbs` approach (no DLL, manifest-only verbs) only works for file types the app *owns* via file type association — it cannot target generic `Directory` or `Directory\Background` keys.
...
The sparse MSIX needs these specific XML namespaces and extensions for a folder context menu handler:
...
The critical manifest extensions for folder context menu:
...
Inno Setup handles the full lifecycle:
1. Installs the `.dll` and `.msix` to `{app}`
2. Imports the self-signed cert into `Cert:\LocalMachine\TrustedPeople` (requires elevation, or user consent prompt)
3. Calls `Add-AppxPackage -Path <msix> -ExternalLocation <install-dir>` via PowerShell inline
4. On uninstall: calls `Get-AppxPackage <name> | Remove-AppxPackage` then removes cert

---

## Certificate Strategy for Public Distribution

Self-signed certificates **work** but require the installer to import the cert into `Cert:\LocalMachine\TrustedPeople` (requires local admin elevation). This is the same approach used by Notepad++, xplorer², and community tools.

For per-user install (no admin):
...
**Recommended path for public distribution:** Self-signed cert + Inno Setup elevates for cert import, then registers the sparse package per-user via PowerShell. This is the established community pattern.
...
project/
  src/
    ClaudeFromHere.cpp        # IExplorerCommand implementation
    ClaudeFromHere.def        # DLL exports
    ClaudeFromHere.rc         # Icon/version resource
  package/
    AppxManifest.xml          # Sparse MSIX manifest
    Assets/
      logo.png                # Required by manifest (44x44 or 150x150)
  installer/
    ClaudeFromHere.iss        # Inno Setup script
    sign-and-pack.ps1         # Build automation: MakeAppx + SignTool
  out/
    ClaudeFromHere.dll        # Built COM DLL (x64)
    ClaudeFromHere.msix       # Signed sparse package
    ClaudeFromHereSetup.exe   # Final installer artifact
```
...
Export-PfxCertificate -Cert "Cert:\CurrentUser\My\<thumbprint>" `
  -FilePath ClaudeFromHere.pfx -Password (ConvertTo-SecureString "pass" -AsPlainText -Force)

# Pack the sparse MSIX (no payload files, only manifest + assets)
MakeAppx.exe pack /o /d .\package\ /nv /p .\out\ClaudeFromHere.msix

# Sign with SHA-256
SignTool.exe sign /fd SHA256 /a /f ClaudeFromHere.pfx /p pass .\out\ClaudeFromHere.msix

# Install (Inno Setup runs these via PowerShell inline)
Import-PfxCertificate -FilePath "ClaudeFromHere.pfx" `
  -CertStoreLocation "Cert:\LocalMachine\TrustedPeople" -Password ...
Add-AppxPackage -Path "ClaudeFromHere.msix" -ExternalLocation "C:\Program Files\ClaudeFromHere\"

# Uninstall
Get-AppxPackage "ClaudeFromHere" | Remove-AppxPackage
```
...
---
*Stack research for: Windows 11 shell extension (Claude From Here)*
*Researched: 2026-04-08*
